...
void run_server_election_tests() {
    // Empty priority list -> no server, regardless of who is online.
    {
        ...
        e.markOnline("A");
        QQ_CHECK(!e.currentServer().has_value());
        QQ_CHECK(!e.isServer("A"));               // no server -> isServer is false
    }
    ...
    // Highest-priority online machine is the server.
    {
        ServerElection e{{"A", "B", "C"}};
        e.setOnline({"A", "B", "C"});
        QQ_CHECK(e.currentServer() == optional<PeerId>("A"));
        QQ_CHECK(e.isServer("A"));
        QQ_CHECK(!e.isServer("B"));               // online, but not the winner
    }
    ...
    // Failover down the list as higher-priority machines drop (user's 1..5 case).
    {
        ...
        QQ_CHECK(e.currentServer() == optional<PeerId>("1"));
        e.markOffline("1");
        QQ_CHECK(e.currentServer() == optional<PeerId>("2"));
        e.markOffline("2");
        e.markOffline("3");
        QQ_CHECK(e.currentServer() == optional<PeerId>("4")); // 4 wins: 1-3 all down
        e.markOffline("4");
        QQ_CHECK(e.currentServer() == optional<PeerId>("5"));
        e.markOffline("5");
        QQ_CHECK(!e.currentServer().has_value());             // all down -> none
    }
    ...
    // Online but ineligible (not on the list) -> never server.
    {
        ...
        e.setOnline({"A", "B"});
        QQ_CHECK(!e.isEligible("A"));
        QQ_CHECK(e.isEligible("B"));
        QQ_CHECK(e.currentServer() == optional<PeerId>("B"));
    }
    ...
    // addToPriority appends at lowest priority and de-dups ("on the list by default").
    {
        ...
        QQ_CHECK(e.priority() == vector<PeerId>({"A", "B"}));
    ...
    // setPriority replaces the entire order.
    {
        ...
        e.setOnline({"A", "B"});
        QQ_CHECK(e.currentServer() == optional<PeerId>("B"));
        QQ_CHECK(e.priority() == vector<PeerId>({"B", "A"}));
    }
    ...
    // markOnline de-dups; markOffline of an absent id is a no-op.
    {
        ...
        QQ_CHECK(e.isOnline("A"));
        e.markOffline("B");                        // absent -> no-op
        QQ_CHECK(e.isOnline("A"));
        QQ_CHECK(!e.isOnline("B"));
        e.markOffline("A");
        QQ_CHECK(!e.isOnline("A"));
    }
}
